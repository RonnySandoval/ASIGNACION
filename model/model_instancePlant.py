...
def obtiene_datos_iniciales():
    ...
    for ids, nombres in zip(BDqueries_before.obtener_id_procesos_secuencia(glo.base_datos), BDqueries_before.leer_procesos_secuencia(glo.base_datos)):
        procesos[nombres] = ids

    for registro in BDqueries_before.leer_modelos_marcas(glo.base_datos):
        objeModelos[registro[1]] = model_classPlant.VehiculoBase(marca=registro[0], modelo=registro[1])

    for vehiculo in BDqueries_before.leer_vehiculos_completos_marcamodelo(glo.base_datos):
    ...
    for tecnico in BDqueries_before.leer_tecnicos(glo.base_datos):
    ...
    for pedido in BDqueries_before.leer_pedidos(glo.base_datos):
    ...
    tiempos["tiempos_mod"]   = BDqueries_before.leer_tiempos_modelos_df(glo.base_datos)
    tiempos["tiempos_veh"] = BDqueries_before.leer_tiempos_vehiculos_df(glo.base_datos)    

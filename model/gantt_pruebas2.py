...
with man.Database('planta_con_ensamble1.db') as db:
    ...
    #df_historicos = crud_historicos.leer_historicos_graficar()
...
def plot_gantt(df, items, tasks, filter1, filter2):
    ...
    df_data["Inicio"] = pd.to_datetime(df_data["Inicio"])
    df_data["Fin"] = pd.to_datetime(df_data["Fin"])
    ...
    def actualizar_gantt(filtro_Filter1, filtro_Filter2, filtro_Task, filtro_Item):
        datos = df_data.copy()
        datos["Text"] = datos["Filter2"] + "<br>" + datos["Filter1"]
        ...
        fig.update_yaxes(autorange="reversed")  # Para que los Items aparezcan de arriba hacia abajo
            
        fig.update_layout(
            margin={"l": 40, "r": 20, "t": 40, "b": 40},
            height=h_plot if h_plot!=0 else 150,  # puedes ajustar el tope

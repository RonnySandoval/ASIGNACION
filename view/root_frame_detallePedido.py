...
class TablaDetallePedido():     #Tabla para pedido
    def __init__(self, contenido, contenedor, laRaiz, bbdd): #Crea latabla y un diccionario con los nombres de los campos
        ...
        __, __, self.iniciaAM, self.terminaAM, self.iniciaPM, self.terminaPM = BDqueries_before.leer_planta_info(bbdd)
    ...
    def llenarTabla(self, bbdd, pedido=None):    # Agregar datos a la tabla
        ...
        if pedido is None:
            self.datos = BDqueries_before.leer_vehiculos_completos_df(bbdd)           # leemos el dataframe en la BBDD
        else:
            self.datos = BDqueries_before.leer_vehiculos_por_pedido_df(bbdd, pedido)  # leemos el dataframe en la BBDD
        print(self.datos)
        ...
        def obtener_texto_tooltip(iid):
            ...
            lecturaHistoricos = BDqueries_before.leer_historico_chasis(bbdd, valores[0])
            lecturaVehiculos  = BDqueries_before.leer_vehiculo_completo(bbdd, valores[0])
